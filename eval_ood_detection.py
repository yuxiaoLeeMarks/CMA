...
def process_args():
    parser = argparse.ArgumentParser(description='Evaluate OOD detection scores for CLIP with CMA defaults',
    ...
    parser.add_argument('--score', default='CMA', type=str, choices=[
        'MCM', 'CMA', 'energy', 'max-logit', 'entropy', 'var', 'maha'], help='score options')
    parser.add_argument('--prompt-template', default='a photo of a {}', type=str,
                        help='prompt template used to convert class names into textual concepts')
    parser.add_argument('--agent-prompts', default=None, type=str,
                        help='custom neutral agent prompts separated by "|" when using CMA')
    parser.add_argument('--agent-prompts-file', default=None, type=str,
                        help='path to a text file with one neutral agent prompt per line when using CMA')
    parser.add_argument('--agent-ratio', default=1.0, type=float,
                        help='ratio k=M/N controlling number of agents for CMA when --num-agents is not provided')
    parser.add_argument('--num-agents', default=None, type=int,
                        help='explicit number of agent prompts to use for CMA (overrides --agent-ratio)')
    ...
    if args.score == 'CMA':
        if args.num_agents is not None and args.num_agents <= 0:
            parser.error("--num-agents must be positive when CMA scoring is selected.")
        if args.num_agents is None and args.agent_ratio <= 0:
            parser.error("--agent-ratio must be positive when CMA scoring is selected.")
...
def main():
    ...
    if args.score == 'maha':
        ...
        if args.score == 'CMA':
            log.debug(f"Using {len(args.agent_prompts_used)} agent prompts.")
            if args.agent_prompts_used:
                preview = args.agent_prompts_used[:min(5, len(args.agent_prompts_used))]
                log.debug(f"Sample agent prompts: {preview}")
